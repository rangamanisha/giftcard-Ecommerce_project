...
import { addRemoveQuantityAction, fetchItemsByCartAction } from "../../actions/cart.actions";
...
function Cart() {
  ...
  const lineItems = get(cartState, "lineItems").length ? get(cartState, "lineItems") : (get(cartState, "fetchedCartItems") || []);
  ...
  useEffect(() => {
    if (authState.isAuthenticated) {
      dispatch(fetchItemsByCartAction({
    ...
    switch (operation) {
      ...
      case "sub":
        if (quantity <= 0) return null;
          ...
          delete _item.quantity;
          if (authState.isAuthenticated) {
            dispatch(addRemoveQuantityAction({ data: { ...payload, action: 'REMOVE' }, item }))
          } else {
            dispatch(
              cartAction.updateLineItem(
                assign({}, _item, { quantity: quantity - 1 })
              ...
              <div className="cart-first-column p-3">
                ...
                <div className="d-flex flex-column justify-content-center align-content-between border border-2 ggp-parent-box rounded p-2 mb-4">
                  ...
                  {!isAuthenticated && (
                    <p>
                      <small>
                        You can also use your Gifti Global Points, Login or Sign up
                        to use your Gift Global Points
                  </small>
                    </p>
                  )}
                            ...
                            <div className="item-details">
                              ...
                              <div className="d-flex justify-content-between align-items-center mt-3 mr-2">
                                <div className="cart-inc-dec-box px-1">
                                  ...
                                  <span className="mx-2">{get(item, "quantity")}</span>
                                  <button
                                    className="btn btn-link"
                                ...
                                <span className="count-name">{get(payment, "country_name")}</span>
